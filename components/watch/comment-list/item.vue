...
const filterComments = computed(() => {
  return props.comment.comments.filter((v) => Boolean(v.comment.trim())).splice(0, 3);
});
...
<template>
  <div class="dark">
    <UCard
      class="flex gap-2 relative overflow-hidden rounded-none cursor-pointer"
      :ui="{
        body: { base: 'w-full', padding: 'p-4 sm:p-4' },
      }"
    >
      <div
        class="absolute top-0 left-0 h-full w-full bg-blue-500 opacity-10"
